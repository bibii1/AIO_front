    ...
    <div class="rowGlobal">
      <form class="form" v-on:submit.prevent="onSubmit" enctype="multipart/form-data">
        ...
        <div class="row">
          ...
          <select
            v-model="model"
            v-else-if=" brand === 'Samsung'&& category==='Smartphone'"
            required
          >
            <option disabled value>Sélectionner le modèle</option>
          ...
          <select
            v-model="model"
            v-else-if=" brand === 'Huawei'&& category==='Smartphone'"
            required
          >
            <option disabled value>Sélectionner le modèle</option>
...
export default {
  ...
  methods: {
    onSubmit() {
      ...
      postService.sendMailContract(
        contract,
        this.user.email,
        this.user.first_name
      );
      postService.createContract(contract).then(() => {
        router.push("/");
      });
    },
    handleFileUpload() {
  ...
  created() {
    postService.getUser(localStorage.getItem("folder_id_user")).then(res => {
      this.user = res.data;
    });
  },
  watch: {
    casse: function(val) {
      this.month_price = this.getMonthPrice(
        val,
        this.vol,
        this.oxydation,
        this.category,
